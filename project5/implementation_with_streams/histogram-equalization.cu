...
__global__ void addH(int * input0, int * input1, int * output) {//kai gia grid megalutero apo 1x1 
	
	// int indexX = threadIdx.x + blockDim.x * blockIdx.x;
	// int indexY = threadIdx.y + blockDim.y * blockIdx.y;
	// int grid_width = gridDim.x * blockDim.x;
	// register int idx = indexY * (grid_width) + indexX;

	output[threadIdx.x] = input0[threadIdx.x] + input1[threadIdx.x];    
    __syncthreads();
}
...
__global__ void calcLut(float* cdf, int* lut, int min, int N_bins, int diff) {
    int idx = threadIdx.x + blockDim.x * blockIdx.x;
    lut[idx] = (int)((cdf[idx] - min) * 255 / diff + 0.5);
    if (lut[idx] < 0) {
        lut[idx] = 0;
    }
