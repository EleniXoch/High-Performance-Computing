...
__global__ void RowGPU(double *d_Dst, const double * d_Src, int imageW, int imageH, int filterR) {
...
int main(int argc, char **argv) {
	...
	mem_time += ((double)(tv4.tv_nsec - tv3.tv_nsec) / 1000000000.0 +
				(double)(tv4.tv_sec - tv3.tv_sec))*1000.0;
	
	//printf("memtime1 = %g\n",mem_time);

	cudaEventRecord(start);
	RowGPU << <grid_dim, block_dim >> > (d_Buffer, d_Input, imageW, imageH, filter_radius);
	...
	ColGPU << <grid_dim, block_dim >> > (d_OutputGPU, d_Buffer, imageW, imageH, filter_radius);
